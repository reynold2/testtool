    ...
    if os.path.isfile(path):
        ...
        if suffix == ".xlsx":
            try:
                ...
                hell(path, old, new)
                excl_e=time.time()
                excl_time=excl_e-excl_b
                print("ok:",path)
                if path in faillist:
                    faillist.remove(path)
                return excl_time
            except:
                print("fail:", path)
                if path in faillist:
...
def wordfilepath(work_dir):
    ...
    return list
def stringrename(string,old,new):
    if type(string):
...
if __name__=='__main__':
    ...
    # print(mianfile("E:\\test\\拟.xlsx","是","我"))
    wordfilepath()
